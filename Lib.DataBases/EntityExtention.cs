...
public static class EntityExtention
{
    public static string CreateBulkInsertQuery(this IEnumerable<Entity> entities)
    {
        ...
        sb.Append("INSERT INTO " + "\"" + table + "\" (" + GetBaseFieldNamesToCSV(first.GetBaseFieldNames()) +
                  ") VALUES ");

        foreach (var entity in entities) sb.Append("(" + entity.GetPostgresQuery() + "),");
    ...
    {
        var sb = new StringBuilder();
        foreach (var name in list) sb.Append(name + " ,");
        sb.Remove(sb.Length - 1, 1);
        return sb.ToString();
    }
